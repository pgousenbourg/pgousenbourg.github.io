...
function y = surface_mean(b,d,t1,t2,manifold,nint)
	if nargin < 6
		nint = 0;
	end
    global_variables;
	...
	% Constrained optimization for the sphere
	switch manifold
		case 'sphere'
			y = surface_mean_constrained(b,d,t1,t2,manifold,nint);
		otherwise
			% minimization of the distance
			w 	= weights_bern(d,t1,t2);
