%% CURVE-RECONSTRUCTION-DE-CASTELJAU-MEAN -
...
function pb = curve_reconstruction_de_casteljau_mean(pb)
    % data
    b      	= pb.control;
    t       = pb.t;
    d       = pb.d;
	...
	for l = 1:m-1
		...
		for i=1:t
		for j=1:t
			curve(dx+i,dy+j,:,:) = de_casteljau_mean(bb,d,vt(i),vt(j),pb.manifold,pb.nint);
    ...
    %% Store
    pb.curve = curve;
    pb.method = 'type3-de-casteljau-averaging-c0';
end
