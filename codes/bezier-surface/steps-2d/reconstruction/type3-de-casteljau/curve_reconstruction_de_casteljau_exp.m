%% CURVE-RECONSTRUCTION-EXP -
%       Reconstrucs the B�zier curve based on the interpolation points and 
%       the control points given in the structure pb. The piecewize B�zier
...
function pb = curve_reconstruction_de_casteljau_exp(pb)    
    % data
    b        = pb.control;
    t        = pb.t;
    d        = pb.d;
	...
	for l = 1:m-1
		...
		for i=1:t
		for j=1:t
			curve(dx+i,dy+j,:,:) = de_casteljau_rectangle(bb,d,vt(i),vt(j));
    ...
    %% Store
    pb.curve = curve;
    pb.method = 'type3-de-casteljau-exp';
end
