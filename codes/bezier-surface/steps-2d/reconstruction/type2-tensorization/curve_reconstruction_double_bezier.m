...
function pb = curve_reconstruction_double_bezier(pb,method)
	global_variables;
	...
	switch method
		case 1
			% Horizontal-vertical
			make_curve = @tensorization_surface_hv;
			methodused = 'type2-tensorization-hv-c0';
		case 2
			% Vertical-horizontal
			make_curve = @tensorization_surface_vh;
			methodused = 'type2-tensorization-vh-c0';
		otherwise
	...
	for l = 1:m-1
		...
		for i=1:t
		for j=1:t
			curve(dx+i,dy+j,:,:) = make_curve(bb,d,vt(i),vt(j));
